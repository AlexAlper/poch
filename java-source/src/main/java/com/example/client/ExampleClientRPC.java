...
public class ExampleClientRPC {
    ...
    private static void logState(StateAndRef<StateUser> state) {
    ...
    public static void main(String[] args) throws ActiveMQException, InterruptedException, ExecutionException {
        ...
        final DataFeed<Vault.Page<StateUser>, Vault.Update<StateUser>> dataFeed = proxy.vaultTrack(StateUser.class);
        final Vault.Page<StateUser> snapshot = dataFeed.getSnapshot();
        final Observable<Vault.Update<StateUser>> updates = dataFeed.getUpdates();
